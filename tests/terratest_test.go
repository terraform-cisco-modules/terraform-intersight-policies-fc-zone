...
func TestFull(t *testing.T) {
	...
	instanceName := fmt.Sprintf("test-policies-fc-zone-%s", uniqueId)
...
{
	"Name":        "{{ .name }}",
	"Description": "{{ .name }} FC Zone Policy.",

	"FcTargetMembers": [
        {
          "ClassId": "fabric.FcZoneMember",
          "Name": "storage-array",
          "ObjectType": "fabric.FcZoneMember",
          "SwitchId": "A",
          "VsanId": 100,
...
`
	...
	iassert.AssertMOComply(t, fmt.Sprintf("/api/v1/fabric/FcZonePolicies/%s", moid), expectedJSONTemplate, vars)
}
